...
public class IBeaconProtocol {
	...
	private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
		...
		byte[] uuid = Arrays.copyOfRange(scanRecord, ADV_PREFIX.length, ADV_PREFIX.length+UUID_LEN);
		IBeacon ibeacon = new IBeacon();
		ibeacon.setUuid(uuid);
		int major = ((scanRecord[offset] << 8) & 0x0000ff00) | (scanRecord[offset+1] & 0x000000ff);
		...
		Log.d("BEACON", "found name: " + device.getName() +  " power: "+ ibeacon.getPowerValue()+ " proximity: "+ ibeacon.getProximity()+ " address: " + device.getAddress() + " rssi: " + rssi);
		
		if(!_arrOrderedIBeacons.contains(ibeacon)){
		    	_arrOrderedIBeacons.add(ibeacon);
		    	Collections.sort(_arrOrderedIBeacons, new IBeaconProximityComparator());
		    	_listener.beaconFound(ibeacon);
		}
